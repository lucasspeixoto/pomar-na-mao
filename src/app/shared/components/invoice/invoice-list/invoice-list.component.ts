...
@Component({
  ...
  imports: [CommonModule, FormsModule, TableDropdownComponent],
...
export class InvoiceListComponent {
  ...
  get allPaginatedSelected(): boolean {
    return (
      ...
      this.paginatedInvoices.every(invoice => this.selected.includes(invoice.id))
    );
  }
  ...
  sortBy(field: 'number' | 'customer' | 'creationDate' | 'dueDate' | 'total'): void {
    this.sort = {
      sortBy: field,
      sortDirection:
        this.sort.sortBy === field && this.sort.sortDirection === 'asc' ? 'desc' : 'asc',
    };
    this.currentPage = 1;
  }
